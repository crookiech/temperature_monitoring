...
class SensorConfigDialog(QDialog):
    ...
    def init_ui(self):
        ...
        for rom_code, sensor in ds18b20_sensors:
            group_box = QGroupBox(f"ROM: {rom_code[-8:]}")
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Статус подключения (крупный и заметный)
    ...
    def connect_to_selected_port(self):
        ...
        self.status_label.setText(f"Подключение к {port_name}...")
        self.status_label.setStyleSheet("QLabel { color: orange; font-weight: bold; font-size: 14pt; background-color: #FFF4CC; border: 2px solid orange; border-radius: 5px; }")
    ...
    def parse_sensor_data(self, data):
        ...
        if "Sensor connected:" in data:
            ...
            if match:
                ...
                self.add_log(f"Подключен DS18B20 ({rom_code})")
            return
        ...
        if "Sensor disconnected:" in data:
            ...
            if match:
                ...
                if rom_code in self.sensors:
                    ...
                    self.add_log(f"Отключен DS18B20 ({rom_code})")
                    self.update_ds18b20_group_title()
    ...
    def add_ds18b20_sensor(self, index, rom_code, temperature):
        ...
        self.add_ds18b20_measurement(sensor_id, temperature, timestamp, sensor, rom_code[-8:])
        
    def add_lm75a_sensor(self, address, temperature):
        timestamp = datetime.now()
        sensor_id = f"LM75A_0x{address}"
